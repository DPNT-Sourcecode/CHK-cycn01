        ...
        for key in sku_dict.keys():
            match key:
                case "A":
                    ...
                    price += self.two_tier_discount(prices["A"], A_special, A_special_2, number)
                case "B":
                    number = sku_dict.get(key)
                    price += self.get_free_by_other(prices["B"], B_special, E_special, number, sku_dict.get("E"))
                case "C":
                    price += sku_dict.get(key) * prices["C"]
                case "D":
                    price += sku_dict.get(key) * prices["D"]
                case "E":
                    # E specials have been applied in B case.
                    price += sku_dict.get(key) * prices["E"]
                case "F":
                    ...
                    number = sku_dict.get(key)
                    price += self.basic_special(prices["F"], F_special, number)
                case _:
                    return -1
